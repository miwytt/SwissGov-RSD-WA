...
# open overlaping annotations files
with open("annotations/fr-annotations-overlap.txt", "r") as f:
    FR_ANNOTATIONS_OVERLAP = f.read().splitlines()
with open("annotations/it-annotations-overlap.txt", "r") as f:
    IT_ANNOTATIONS_OVERLAP = f.read().splitlines()
with open("annotations/de-annotations-overlap.txt", "r") as f:
    DE_ANNOTATIONS_OVERLAP = f.read().splitlines()

data_dir = Path(__file__).parent.parent / "data"

if args.lang:
    test_gold_path_ann1 = data_dir / "annotations" / "gold_labels_overlap" / "annotations1"/ f"gold_admin_{args.lang}{'_short' if args.short else ''}.jsonl"
    test_gold_path_ann2 = data_dir / "annotations" / "gold_labels_overlap" / "annotations2"/ f"gold_admin_{args.lang}{'_short' if args.short else ''}.jsonl"
...
for i, (page, pages) in enumerate(ALIGNED_PAGES.items()):
    ...
    # Each overlap list contains ENGLISH filenames; decide per group whether it is in overlap.
    en_base = None
    for p in pages + [page]:
        if "_en_" in p:
            en_base = p.replace('https___www.', '').replace('.html', '.jsonl')
            break

    # If we can't find the English page, or it's not in the overlap list for this language, skip.
    if en_base is None:
        continue
    if args.lang == "de" and en_base not in DE_ANNOTATIONS_OVERLAP:
        continue
    if args.lang == "fr" and en_base not in FR_ANNOTATIONS_OVERLAP:
        continue
    if args.lang == "it" and en_base not in IT_ANNOTATIONS_OVERLAP:
        continue
    ...
    # If we couldn't load both English and the requested other-language text, skip this page
    if en_text is None or args.lang not in other_texts:
        continue
    ...
    try:
        for annotator in [1, 2]:
            with open(f"annotations/annotations_raw/annotations{annotator}/{args.lang}/{page.replace('https___www.', '').replace('.html', '.jsonl')}", "r", encoding="utf-8") as f:
                
                if en_text is None or other_texts[args.lang] is None:
                    raise FileNotFoundError(f"File {f.name} does not exist")
                    continue
                ...
                if args.lang == "fr" or args.lang == "it" or args.lang == "de":
                    ...
                    if not args.short:
                        ...
                        "annotator_tag": 1 if annotator == 1 else 2,
                    })
                    else:
...
# write gold labels for annotator 1
os.makedirs(test_gold_path_ann1.parent, exist_ok=True)
os.makedirs(test_gold_path_ann2.parent, exist_ok=True)

with jsonlines.open(test_gold_path_ann1, "w") as f:
    doc_counter = -1
    current_page = None
    for text_pair in text_pairs:

        if text_pair["id"] in LIST_TO_DROP or text_pair["annotator_tag"] == 2:
            continue
    ...
    print(f"Wrote {len(text_pairs)} text pairs to {test_gold_path_ann1}")


# write gold labels for annotator 2
with jsonlines.open(test_gold_path_ann2, "w") as f:
    doc_counter = -1
    current_page = None
    for text_pair in text_pairs:

        if text_pair["id"] in LIST_TO_DROP or text_pair["annotator_tag"] == 1:
            continue
    ...
    print(f"Wrote {len(text_pairs)} text pairs to {test_gold_path_ann2}")
